document.addEventListener('DOMContentLoaded', function () {
  ...
  // Set the initial history state (categories view)
  history.replaceState({ view: 'categories' }, '', '');

  // Back button support
  window.addEventListener('popstate', function (e) {
    if (e.state && e.state.view === 'categories') {
  ...
  function fetchData() {
    fetch(endpoint)
      ...
      .catch(err => {
        galleryGrid.innerHTML = `<p style="color: var(--accent-red);">⚠️ Failed to load gallery.</p>`;
  ...
  function renderGallery(items, categoryName = '') {
    // Push gallery view to browser history
    ...
    if (categoryName) {
      const backBtn = `<button class="view-btn" style="margin-bottom: 20px;" onclick="history.back()">← Back to Categories</button>`;
  ...
  function addToCart(item) {
    ...
    const exists = storedCart.some(i => i.img === item.img); // remove by img only
    if (!exists) {
